...
<div class="form">
  ...
  <div style="float:left;width:50%;padding:0px 10px;">
		<?php if ( !$model->isNewRecord ) { 
			...
			$jeniskompetensi = $model->routeJeniskompetensi()
		?>
		<table>
		<?php
			$nowjkom = '';
			foreach ($jeniskompetensi as $jk=>$value_jkomp) {
				$jkom = $value_jkomp->id;
				
			?>
				<tr>
					<td class="rotatex" colspan="2" style="background:#aaa;font-weight:bold;width:250px;border-bottom:1px solid #eee;padding:5px 10px;"><?php echo $value_jkomp->name?></td>
					
				</tr>
        <?php foreach ( $value_jkomp->kompetensi as $kom=>$valKomp){ ?>
          <tr>
          <td class="rotatex" style="width:250px;border-bottom:1px solid #eee;padding:5px 0px;"><?php echo $valKomp->name?></td>
          <td class="rotatex" style="border-bottom:1px solid #eee;padding:5px 0px;">
            <input type="text" name="Kompetensiskj[<?php echo $jkom?>][<?php echo $valKomp->id?>]" value="<?php echo $itemkompetensi[$jkom][$valKomp->id]?>">
          </td>
          </tr>
        <?php } ?>
  ...
  <div style="float:left;width:50%;padding:0px 10px;">
		<?php if ( !$model->isNewRecord ) { 
			...
			$kompetensi = $model->routeKompetensi();
		?>
		<table>
		<?php
			$nowjkom = '';
