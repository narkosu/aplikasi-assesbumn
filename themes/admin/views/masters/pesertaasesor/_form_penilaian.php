...
<?php $form=$this->beginWidget('CActiveForm', array(
	...
	<div>
		<table>
			...
			<Tr>
				...
				<td>
					<?php
					$list=CHtml::listData(Masterskj::model()->findAll('departement_id = :dept AND type = 1',array(':dept'=>$this->module->current_departement_id)), 'id', 'skj_name');
					?>
  ...
  ?>
	...
	<div id="uraianKompetensi">
		...
		<?php if ( !$model->isNewRecord ) {
			echo (empty($output['uraianKompetensi']) ? '' : $output['uraianKompetensi']);
		} ?>
	...
	<?php if ( !$model->isNewRecord ) {
			echo (empty($output['ringkasanKompetensi']) ? '' : $output['ringkasanKompetensi']);
	} ?>
	</div>
	<div style="clear:both;"></div>
  ...
  $result['PC'] = ( empty($result['PC'])?'':$result['PC']);
	...
	<?php if ( !$model->isNewRecord ) {
		echo (EMPTY($output['saranpengembangan']) ? '' :$output['saranpengembangan']);
	} ?>
	</div>
	<div style="clear:both;"></div>
...
$(document).delegate('.tulisnilai','click',function(){
  ...
  var depid = '".$depid."';
	var nilai = $(this).val();
  var defaultNilai = $('#total_'+id_komp).attr('datadefault');
  var gapnilai;
  
  if ( nilai <= defaultNilai ){
		gapnilai =  nilai - defaultNilai;
	}
	if ( nilai > defaultNilai ){
		gapnilai =  nilai - defaultNilai;
	}
  ...
  $('#total_'+id_komp).val(gapnilai); // persentase
