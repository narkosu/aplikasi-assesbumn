...
drawChartPriv = function (id, repos, align, fit)
{
	...
	// Draw the boxes:
	for (i = 0; i < nodes.length; i++){
		drawNode(ctx, i, nodes[i].hpos, nodes[i].vpos, boxWidth, boxHeight, nodes[i].txt, nodes[i].bold, nodes[i].linecolor, nodes[i].fillcolor, nodes[i].textcolor, nodes[i].textfont, nodes[i].textsize, nodes[i].valign, nodes[i].img);
		drawNode(ctx, i);
	}
...
drawNode = function(ctx, i)
{
	...
	if (img){
		...
		if (img.width > 0){
			...
			if (maxy > height - 2 * radius){
				maxx = img.width * (height - 2 * radius) / img.height;
				maxy = height - 2 * radius;
			}

			// Resize if image too width, even after previous resize:
			if (maxx > width - 2 * radius){
				maxy = img.height * (width - 2 * radius) / img.width;
				maxx = width - 2 * radius;
			}
		}else{
			if (width > height){
		...
		// Horizontal offset:
		xPic = radius;
		if (imgalign == 'c') xPic = (width - 2 * radius - maxx) / 2 + radius;
		if (imgalign == 'r') xPic = width - maxx - radius;
