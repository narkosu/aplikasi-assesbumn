...
class Penilaian extends CActiveRecord
{
  ...
  public static function model($className=__CLASS__)
	...
	public function rules()
	{
		...
		return array(
			...
			array('id, type_competence, departement_id, assessor_id, peserta_id, skj_id, created_at', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'masterskj'=>array(self::BELONGS_TO,'Masterskj','skj_id'),
      ...
      'peserta' => array(self::BELONGS_TO, 'Peserta', 'peserta_id'),
		);
	}
  ...
  static public function routeLevelSaranPengembangan($type_competence = 1,$params = array()){
    
    switch($type_competence){
      case Masterskj::SOFT_COMPETENCE:
        return LevelSaranpengembangan::model()
                ->detail($params['departement_id'],
                          $params['kompetensi_id'],
                          $params['jenispengembangan_id'],
                          $params['level']);
        break;
      case Masterskj::HARD_COMPETENCE:
        $xx = LevelSaranpengembanganHard::model()
                ->detail($params['departement_id'],
                          $params['kompetensi_id'],
                          $params['jenispengembangan_id'],
                          $params['level']);
