...
class Itemskj extends CActiveRecord
{
  ...
  public function routeKompetensi(){
    switch($this->skj->type){
      case Masterskj::SOFT_COMPETENCE:
        return Kompetensi::model()->findAll('departement_id = "'.$this->skj->departement_id.'"');
        break;
      case Masterskj::HARD_COMPETENCE:
        return KompetensiHard::model()
                    ->findAll('departement_id = "'.$this->skj->departement_id.'" AND
                              skj_id = "'.$this->skj_id.'"');
        break;
    } 
  }
  
  public function routeJeniskompetensi(){
    switch($this->skj->type){
      case Masterskj::SOFT_COMPETENCE:
        return Jeniskompetensi::model()
                    ->findAll('departement_id = "'.$this->skj->departement_id.'" AND
                              skj_id = "'.$this->skj_id.'"
                      ');
        break;
      case Masterskj::HARD_COMPETENCE:
        return JeniskompetensiHard::model()
                    ->findAll('departement_id = "'.$this->skj->departement_id.'" AND
                              skj_id = "'.$this->skj_id.'" AND unitkerja_id = "'.$this->unitkerja_id.'"');
        break;
    } 
  }
